...
function drawHead($src) {
	if($src == "main") { //Which page is it called from?
		...
		$backlink = "<a href='index.php'>Startseite</a>";
	}

	if(isset($_SESSION['login']) && $_SESSION['login']) { //user is logged in
		echo '<form name="logout" action="'.$_SERVER['PHP_SELF'].'" method="post">';
			echo "<span>Hallo ".$_SESSION['name']." - ".$backlink." - </span>";
			echo '<input type="submit" class="logout" name="logout" value="Ausloggen" /> 
			</form>';
		echo "<br>";
	} else  { //login
	  echo '<form name="login" action="'.$_SERVER['PHP_SELF'].'" method="post">
				...
				<input type="submit" class="logout login" value="Einloggen" /> 
			</form>
			<br>';
	}
}
